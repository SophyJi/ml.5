...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node * min_element_impl(Node *node) {
    ...
    if(!node->left) {
      ...
      // T min = std::min(node->datum, max_element_impl(node->left)->datum);
      // return &min;
  ...
  static Node * max_element_impl(Node *node) {
    ...
    if(!node->right) {
      ...
      // T max = std::max(node->datum, max_element_impl(node->right)->datum);
      // return &max;
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
   return (!node || ((!node->left||(less(max_element_impl(node->left)->datum, 
      node->datum) && check_sorting_invariant_impl(node->left, less))) &&
    ...
    //     return true;
    //   }
    //   // check_sorting_invariant(node->left, less);
    // }
    // if(node->right) {
